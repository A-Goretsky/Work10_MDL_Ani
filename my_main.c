...
struct vary_node ** second_pass() {
  extern int num_frames;
  struct vary_node** thing = (struct vary_node**)malloc(sizeof(struct vary_node) * num_frames);
...
void my_main() {
  ...
  if (num_frames > 1) {
      for (int fra = 0; fra < num_frames; fra++) {
          printf("Frame %d\n", fra);
          struct vary_node * head = knobs[fra];
          while (head != NULL) {
              set_value(lookup_symbol(head->name), head->value);
              head = head->next;
          }
      ...
      for (i=0;i<lastop;i++) {
        ...
        case SETKNOBS:;
            struct vary_node * head = knobs[fra];
            int ctr1;
            for (ctr1 = 0; ctr1 < lastsym; ctr1++) {
                if (symtab[ctr1].type == SYM_VALUE)
                    symtab[ctr1].s.value = op[i].op.setknobs.value;
            }
            break;
    	case SPHERE:
        ...
        valKnob = 1;
    	  ...
    	  if (op[i].op.move.p != NULL)
            ...
            if (op[i].op.move.p != NULL && lookup_symbol(op[i].op.move.p->name) != NULL) {
                valKnob = lookup_symbol(op[i].op.move.p->name)->s.value;
            }
    	  tmp = make_translate( op[i].op.move.d[0] * valKnob,
    				op[i].op.move.d[1] * valKnob,
    				op[i].op.move.d[2] * valKnob);
    	  matrix_mult(peek(systems), tmp);
        ...
        valKnob = 1;
          ...
          if (op[i].op.scale.p != NULL && lookup_symbol(op[i].op.scale.p->name) != NULL){
	         valKnob = lookup_symbol(op[i].op.scale.p->name)->s.value;
	      }
    	  tmp = make_scale( op[i].op.scale.d[0] * valKnob,
    			    op[i].op.scale.d[1] * valKnob,
    			    op[i].op.scale.d[2] * valKnob);
    	  matrix_mult(peek(systems), tmp);
    	  copy_matrix(tmp, peek(systems));
    	  tmp->lastcol = 0;
    	  break;
        ...
        valKnob = 1;
          ...
          if (op[i].op.rotate.p != NULL && lookup_symbol(op[i].op.rotate.p->name) != NULL){
    	      valKnob = lookup_symbol(op[i].op.rotate.p->name)->s.value;
    	  }
    	  theta =  op[i].op.rotate.degrees * (M_PI / 180) * valKnob;
        ...
        char dir[100];
        sprintf(dir, "anim/%s%03d", name, fra);
